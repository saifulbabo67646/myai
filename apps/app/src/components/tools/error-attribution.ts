...
export function attributeChatToolError(errorText: string): ToolErrorAttribution | null {
  ...
  if (
    errorText.includes("myai stopped waiting after")
    ...
    return confirmed(
      "myai timeout",
      "myai created this deadline. The external operation may still have completed, so verify its state before retrying.",
    )
  }
  ...
  if (
    ...
    return confirmed("Blocked by myai", "myai blocked the request before it was sent.")
  }
