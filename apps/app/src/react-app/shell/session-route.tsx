...
function describeTaskCreateError(error: unknown) {
  ...
  if (
    ...
    return "OpenCode is unavailable for this workspace. Retry once it restarts, or restart myai if the problem continues.";
  }
  return message;
}
...
export function SessionRoute() {
  ...
  const handleChatFirstTask = useCallback((prompt: string, attachments?: ComposerAttachment[]) => {
    void (async () => {
      ...
      const folder = await joinDesktopPath(home, "myai Chat").catch(() => "");
      if (!folder) {
        handleOpenCreateWorkspace();
        return;
      }
