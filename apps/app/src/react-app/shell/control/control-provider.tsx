  ...
  const executeCommand = useCallback(async (
    ...
    if (request.expectedRevision !== undefined && request.expectedRevision !== revision) {
      return {
        ok: false,
        id: request.id,
        error: `myai context changed from revision ${request.expectedRevision} to ${revision}.`,
        code: "conflict",
        revision,
      };
    }
...
export function OpenworkRouteControlActions() {
  ...
  const actions = useMemo<OpenworkControlAction[]>(() => [
    ...
    },
    {
      ...
      label: "What can myai do?",
      description: "List the main capabilities of myai.",
      kind: "query",
      ...
      execute: () => ({
        capabilities: [
          ...
          { id: "voice", label: "Voice mode", description: "Talk to myai with real-time voice using OpenAI Realtime." },
