 ...
 *      localhost ports/tokens into myai settings.
 *
      ...
      try {
        ...
        const startServerWithoutDesktopWorkspace = async () => {
          ...
          if (!isOpenworkServerInfoLike(serverInfo) || !isOpenworkServerReady(serverInfo)) {
            setError("OpenWork server did not finish starting. Please restart myai.");
            return;
          }
        ...
        if (isElectronRuntime()) {
          ...
          if (boot.ok === false) {
            setError(boot.error || "Failed to start myai runtime");
            return;
          }
