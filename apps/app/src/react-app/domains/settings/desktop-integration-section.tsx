...
function statusDescription(status: DesktopIntegrationStatus) {
  if (status.state === "integrated") {
    return "myai is in your application launcher and handles openwork:// browser callbacks.";
  }
  if (status.state === "managed_externally") {
    return "This AppImage is integrated by another app. myai will leave its launcher untouched.";
  }
  if (status.state === "needs_repair" && status.ownership === "external") {
    return status.issues.includes("desktop-entry")
      ...
      : "Another app manages this AppImage. Select its launcher for myai browser callbacks.";
  }
