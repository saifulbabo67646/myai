...
export function useDebugViewModel(options: UseDebugViewModelOptions) {
  ...
  const onRevealElectronMigrationBackup = useCallback(async () => {
    ...
    try {
      ...
      if (!appBundlePath) {
        setElectronMigrationStatus("Could not resolve the current myai.app bundle path.");
        return;
      }
      await revealDesktopItemInDir(`${appBundlePath}.migrate-bak`);
      setElectronMigrationStatus("Requested Finder reveal for myai.app.migrate-bak. The backup exists after an install handoff completes.");
    } catch (error) {
      setElectronMigrationStatus(error instanceof Error ? error.message : safeStringify(error));
    }
  }, []);
  ...
  const onOpenResetModal = useCallback(
    (mode: ResetModalMode) => {
      ...
      const message =
        mode === "all"
          ? "Reset ALL myai app data? Open sessions and workspaces will be removed."
      ...
      void resetOpenworkState(mode)
        .then(async () => {
          ...
          setResetStatus(
            mode === "all"
              ? "Reset myai state. Restart the app to see changes."
