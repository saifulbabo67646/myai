...
export function createConnectionsStore(options: {
    ...
    try {
      ...
      if (!fallbackOnError) {
        throw error instanceof Error
          ? error
          : new Error("Computer Use helper app is unavailable. Restart myai or reinstall the app.");
      }
  ...
  async function refreshMcpServers() {
    ...
    try {
      ...
      // Runtime-DB MCPs (source "config.remote") only exist on the myai
    ...
    try {
      ...
      if (entry.managedBy === "openwork-connect") {
        if (slug !== CLOUD_MCP_SERVER_NAME) {
          throw new Error("myai Connect MCP metadata is invalid.");
        }
        if (!canUseOpenworkServer || !openworkClient || !openworkWorkspaceId) {
      ...
      if (entryType === "remote") {
        if (!resolvedUrl) {
          throw new Error("Missing MCP URL. Is the myai desktop app running?");
        }
