...
export function cloudMcpFailureStageLabel(input: {
  ...
  if (code.includes("tool_ids") || code.includes("client_registration")) return "myai components need updating";
...
export function cloudMcpRecommendedAction(input: {
  ...
  if (code.includes("tool_ids") || code.includes("client_registration")) return "Update myai, then retry.";
  if (code === "extensions_plugin_missing") return "Reload the agent so myai instructions are current.";
