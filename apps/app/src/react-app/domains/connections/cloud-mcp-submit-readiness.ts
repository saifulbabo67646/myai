...
function genericSubmissionIssue(input?: {
  ...
  return {
    ...
    message: input?.message ?? "myai could not verify connected service tools for the selected model.",
  };
}
...
function authResolutionIssue(input?: { timedOut?: boolean }): CloudMcpSubmissionIssue {
  return genericSubmissionIssue({
    ...
    message: input?.timedOut
      ? "myai timed out while restoring connected service access."
      : "myai could not finish restoring connected service access.",
  ...
  if (!projection.checked) {
    return {
      ...
      issue: genericSubmissionIssue({
        ...
        stage: "provider_projection",
        message: "myai could not read tool capability information for the selected provider and model.",
      ...
      issue: genericSubmissionIssue({
        ...
        message: modelMissing
          ...
          : toolCallingUnavailable
            ...
            : "myai could not confirm that the selected model supports tool calling.",
  ...
  if (projection.source !== "experimental_tool") {
    return {
      ...
      issue: genericSubmissionIssue({
        ...
        stage: "provider_projection",
        retryable: false,
        message: "myai received an unsupported tool capability result for the selected provider and model.",
...
function timeoutIssue(): CloudMcpSubmissionIssue {
  return genericSubmissionIssue({
    ...
    message: "myai timed out while preparing connected service tools.",
  });
}
  ...
  return {
    ...
    issue: timedOut
      ...
      : genericSubmissionIssue({
          ...
          message: "myai could not check connected service tools before sending.",
        }),
  };
}
