...
function missingContextIssue(input: {
  ...
  if (!input.client || !input.workspaceId) {
    return {
      ...
      retryable: true,
      message: "myai could not resolve the workspace server before checking connected service tools.",
  ...
  return {
    ...
    stage: "provider_projection",
    retryable: false,
    message: "myai could not verify connected service tools for this submission.",
