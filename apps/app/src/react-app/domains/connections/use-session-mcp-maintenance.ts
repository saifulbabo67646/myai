...
function genericCloudMcpMaintenanceIssue(input?: {
  ...
  return {
    ...
    message: input?.message ?? "myai could not verify connected service tools for this workspace.",
  };
}
  ...
  if (result.status === "skipped") {
    ...
    if (result.skippedReason === "mint_failed") {
      return failedCloudMcpBackgroundSync({
        ...
        message: "myai could not refresh Cloud authentication for connected service tools.",
      });
    }
  }
  return failedCloudMcpBackgroundSync({
