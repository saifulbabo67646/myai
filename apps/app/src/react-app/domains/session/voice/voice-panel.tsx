...
const DEFAULT_TEXT_COMMAND = "Summarize the current myai session and put the next step in the composer.";
...
const TOOL_LABELS: Record<string, string> = {
  openwork_snapshot: "Checking myai",
...
function humanToolLabel(toolName?: string) {
  if (!toolName) return "myai action";
...
async function executeOpenWorkTool(name: string, args: Record<string, unknown>) {
  ...
  if (!control) return { ok: false, error: "myai control surface is not available." };
  ...
  return { ok: false, error: `Unknown myai voice tool: ${name}` };
}
...
export function VoicePanel(props: VoicePanelProps) {
    ...
    setVoiceRuntimeSnapshot((current) => ({
      ...
      statusText: text ?? (
        nextStatus === "connecting" ? "Connecting to OpenAI Realtime..." :
          nextStatus === "listening" ? "Listening. Ask myai to act." :
            nextStatus === "speaking" ? "myai is speaking..." :
  ...
  const connectRealtime = useCallback(async (audioInput = true) => {
    ...
    if (audioInput) {
      ...
      if (!macPermissionGranted) throw new Error("macOS denied microphone access. Enable myai in System Settings > Privacy & Security > Microphone, then restart myai.");
    ...
    addEntry("system", `Realtime connected with ${realtimeSession.model} and ${realtimeSession.tools.length} myai tools.`);
    ...
    <div ref={panelRef} className="flex h-full min-h-0 flex-col bg-background">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
        <div className="min-w-0">
          ...
          <div className="truncate text-xs text-muted-foreground">Realtime voice over myai UI MCP controls</div>
        </div>
