...
function EnterpriseActivationPage() {
  ...
  const exchangeConfirmedGrant = async (grant: string, baseUrl: string) => {
    ...
    setAuthError(null);
    setServerError(null);
    setStatusMessage("Finishing myai Enterprise sign-in…");
    try {
      ...
      const result = await exchangeHandoffAndSignIn(grant, {
        ...
        fallbackErrorMessage: "myai Enterprise did not return a session token.",
      });
      if (!result.ok) {
      ...
      setStatusMessage(null);
      setAuthError(
        error instanceof Error ? error.message : "Unable to finish myai Enterprise sign-in.",
      );
    } finally {
      setAuthBusy(false);
    }
  ...
  const confirmServer = async () => {
    ...
    try {
      ...
      setStatusMessage("Finish signing in in your browser, then return to myai.");
    } catch (error) {
      setStatusMessage(null);
      setServerError(
            ...
            {pendingConfirmation ? (
              <section className="space-y-3 rounded-xl border border-border bg-muted/30 p-4">
                <p className="text-sm leading-6 text-foreground">
                  Connect this app to <strong className="break-all font-semibold">{pendingConfirmation.baseUrl}</strong>? This signs you in with that organization and binds myai Enterprise to it.
                </p>
