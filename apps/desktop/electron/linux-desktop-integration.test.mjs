...
async function createHarness(options = {}) {
  ...
  const appImagePath = options.appImagePath ?? path.join(root, "myai AppImage");
...
describe("Linux AppImage desktop integration", () => {
  it("quotes AppImage paths for Desktop Entry Exec fields", () => {
    ...
    assert.match(entry, /^MimeType=x-scheme-handler\/myai;$/m);
    assert.match(entry, /^StartupWMClass=bd\.myai\.app$/m);
  });
  ...
  it("installs its launcher, icons, and myai handler", async () => {
    ...
    assert.match(desktopEntry, /^X-Myai-Managed=true$/m);
  ...
  it("detects a moved AppImage and repairs the launcher for the new path", async () => {
    ...
    const movedPath = path.join(first.root, "Applications", "myai.AppImage");
  ...
  it("silently repairs an owned launcher after a self-update lands a new filename", async () => {
    ...
    const updatedPath = path.join(harness.root, "myai-linux-x86_64-0.18.8.AppImage");
    ...
    assert.match(entry, /^X-Myai-Version=0\.18\.8$/m);
  });
  ...
  it("keeps a failed silent repair quiet and leaves it to Settings", async () => {
    ...
    const movedPath = path.join(harness.root, "myai-linux-x86_64-0.18.8.AppImage");
  ...
  it("recognizes an external manager and does not duplicate its integration", async () => {
    const managerDesktopId = "it.mijorus.gearlever.myai.desktop";
...
MimeType=x-scheme-handler/myai;
`);
...
`;
  ...
  it("registers an existing manager launcher without creating a duplicate", async () => {
    const managerDesktopId = "appimagelauncher-myai.desktop";
