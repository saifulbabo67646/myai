...
async function repairIncompleteChains(options) {
  ...
  if (chainRepair.disabled === true || String(env.OPENWORK_DISABLE_CHAIN_REPAIR ?? "").trim() === "1") {
    if (typeof logInfo === "function") logInfo("myai runtime: chain repair disabled by OPENWORK_DISABLE_CHAIN_REPAIR.");
  ...
  if (origins.length === 0) {
    if (!chainRepair.origins && !String(env.OPENWORK_CHAIN_REPAIR_ORIGINS ?? "").trim() && typeof logInfo === "function") {
      logInfo("myai runtime: chain repair skipped: no activation record.");
    }
    return { pems: [], timedOut: false };
  }
  ...
  if (typeof fetchImpl !== "function") {
    if (typeof logInfo === "function") {
      for (const origin of origins) logInfo(`myai runtime: chain repair skipped for ${origin}: fetch unavailable`);
    }
    return { pems: [], timedOut: false };
  }
  ...
  const run = async () => {
    ...
    for (const origin of origins) {
      ...
      if (strictError === null) {
        if (typeof logInfo === "function") logInfo(`myai runtime: chain ok for ${origin}`);
        continue;
      }
      if (strictError !== "UNABLE_TO_VERIFY_LEAF_SIGNATURE") {
        if (typeof logInfo === "function") logInfo(`myai runtime: chain repair skipped for ${origin}: ${strictError}`);
        continue;
      }

      const leafState = await introspectLeafCertificate(origin, tlsConnectImpl);
      if (!leafState) {
        if (typeof logInfo === "function") logInfo(`myai runtime: chain repair skipped for ${origin}: certificate introspection failed`);
        continue;
      }
      if (!leafState.leafOnly) {
        if (typeof logInfo === "function") logInfo(`myai runtime: chain repair skipped for ${origin}: served chain includes an intermediate`);
        continue;
      }

      const issuerUrls = caIssuerUrls(leafState.leaf);
      if (issuerUrls.length === 0) {
        if (typeof logInfo === "function") logInfo(`myai runtime: chain repair skipped for ${origin}: no CA Issuers AIA URL`);
        continue;
      }
      ...
      for (const url of issuerUrls) {
        ...
        if (reason) {
          if (typeof logInfo === "function") logInfo(`myai runtime: chain repair refused for ${origin}: ${reason}`);
          continue;
        }
        pems.push(intermediate.toString());
        repaired = true;
        if (typeof logInfo === "function") {
          logInfo(`myai runtime: chain repaired for ${origin}: added "${certificateCommonName(intermediate)}"`);
        }
        break;
      }
      if (!repaired && typeof logInfo === "function") {
        logInfo(`myai runtime: chain repair skipped for ${origin}: no usable AIA issuer certificate`);
      }
    }
    return { pems, timedOut: false };
  };
...
async function resolveSystemCa({
  ...
  if (Object.prototype.hasOwnProperty.call(env, "NODE_EXTRA_CA_CERTS")) {
    if (typeof logInfo === "function") {
      logInfo("myai runtime: NODE_EXTRA_CA_CERTS is already set; skipping system CA bundle export.");
    }
    try {
  ...
  try {
    ...
    if (typeof logInfo === "function") {
      logInfo(`myai runtime: system CA bundle sources ${summarizeSystemCaSources(bundle.sources)}`);
    }
    let repairedPems = [];
    try {
      ...
      if (repaired.timedOut && typeof logInfo === "function") {
        logInfo("myai runtime: chain repair skipped: timed out");
      }
    } catch {
      repairedPems = [];
    }
...
export function createRuntimeManager({
  ...
  async function issueOwnerToken(baseUrl, hostToken) {
    const payload = await fetchJson(
      `${baseUrl.replace(/\/+$/, "")}/tokens`,
      {
        ...
        body: JSON.stringify({ scope: "owner", label: "myai desktop owner token" }),
      },
      5000,
    );
  ...
  async function startOpenworkServerInner(options) {
    ...
    if (!embeddedPath) {
      throw new Error(`Cannot find myai embedded server bundle. Checked: ${candidates.join(", ")}`);
    }
  ...
  async function engineInstall() {
    if (process.platform === "win32") {
      return {
        ...
        stdout: "",
        stderr:
          "Guided install is not supported on Windows yet. Install the myai-pinned OpenCode version manually, then restart myai.",
      };
    }
