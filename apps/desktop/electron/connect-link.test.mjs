...
test("extracts only dedicated desktop connect links", () => {
  assert.equal(extractConnectLinkToken("myai://connect?token=a.b.c"), "a.b.c");
  assert.equal(extractConnectLinkToken("myai-dev://connect?token=a.b.c"), "a.b.c");
  assert.equal(extractConnectLinkToken("openwork://connect?token=a.b.c"), "a.b.c");
  assert.equal(extractConnectLinkToken("myai:///connect?token=a.b.c"), "a.b.c");
  assert.equal(extractConnectLinkToken("myai://den-auth?grant=x"), null);
...
test("verifies an exact signed organization target end to end", () => {
  ...
  const result = verifyConnectLinkUrl(
    `myai://connect?token=${encodeURIComponent(token)}`,
...
test("resolves a keyless exchange through the exact HTTPS Den endpoint", async () => {
  ...
  const rawUrl = `myai://connect?code=${code}&apiBaseUrl=${encodeURIComponent(apiBaseUrl)}`;
...
test("fails closed for ambiguous, insecure, mismatched, expired, and replayed exchanges", async () => {
  ...
  const insecure = `myai://connect?code=${code}&apiBaseUrl=${encodeURIComponent("http://den.example.com")}`;
