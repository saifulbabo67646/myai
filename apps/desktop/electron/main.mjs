...
async function resolveArchitectureInfo() {
  ...
  const assetName = `myai-${platformDownloadSlug()}-${downloadAssetArch(targetArch)}-${version}.${downloadAssetExtension()}`;
  ...
  return argv
    ...
    .filter(
      (entry) =>
        ...
        (!app.isPackaged && (entry.startsWith("myai-dev://") || entry.startsWith("openwork-dev://"))) ||
...
<html>
  ...
  <body>
    <main>
      ...
      <div class="title">Stopping myai services</div>
...
function assertDesktopActivation() {
  if (desktopActivationRequired(
    ...
    throw new Error("myai must be activated from your Den portal before this command is available.");
  }
}
...
if (!app.requestSingleInstanceLock()) {
  if (isDevMode && !app.isPackaged) {
    console.error(`[openwork] Another myai dev instance already holds this profile directory:
