...
export function createUiControlServer({ appName, appIdentifier, getWindow }) {
  ...
  async function runOpenworkControlCommand(command, args = {}) {
    ...
    if (command === "snapshot") {
      return evaluateOpenworkControl(`(async () => {
        ...
        if (!control) return { ok: false, error: "myai control surface is not available yet." };
    ...
    if (command === "execute") {
      return evaluateOpenworkControl(`(async () => {
        ...
        if (!input || typeof input.actionId !== "string" || !input.actionId.trim()) {
          return { ok: false, error: "Missing myai actionId." };
        }
        control.setEnabled?.(true);
        return control.execute(input.actionId, input.args ?? {});
      })()`);
    }
    return { ok: false, error: `Unknown myai control command: ${command}` };
  }
  ...
  async function start() {
    ...
    if (!port) throw new Error("Could not start myai UI control bridge.");
