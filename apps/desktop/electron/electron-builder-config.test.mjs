...
describe("Electron distribution configs", () => {
  it("uses a stable Linux desktop identity and ships integration icons", async () => {
    ...
    assert.equal(packageMetadata.desktopName, "bd.myai.app");
  ...
  it("keeps the public artifact and protocol unchanged", async () => {
    ...
    assert.equal(config.appId, "bd.myai.app");
    assert.equal(config.productName, "myai");
    assert.deepEqual(config.protocols[0].schemes, ["myai", "openwork"]);
    assert.equal(config.artifactName, "myai-${os}-${arch}-${version}.${ext}");
  });
  ...
  it("defines an enterprise flavor with the standard app identity and release provider", async () => {
    ...
    assert.equal(config.extends, "./electron-builder.base.yml");
    assert.equal(config.appId, "bd.myai.app");
    assert.equal(config.productName, "myai Enterprise");
    assert.equal(config.extraMetadata.openworkDistribution, "enterprise");
    assert.equal(config.protocols[0].schemes[0], "myai");
    assert.equal(config.publish[0].provider, "github");
    assert.equal(config.publish[0].owner, "saifulbabo67646");
    assert.equal(config.publish[0].repo, "myai");
    assert.equal(config.publish[0].channel, "enterprise");
    assert.equal(
      config.artifactName,
      "myai-enterprise-${os}-${arch}-${version}.${ext}",
    );
  });
