...
test("updates both the live window icon and Windows taskbar identity", async () => {
  ...
  await applyWindowsTaskbarIcon(window, {
    ...
    relaunchCommand: "C:\\Program Files\\myai\\myai.exe",
    relaunchDisplayName: "myai",
  ...
  assert.deepEqual(calls, [
    ...
    ["setAppDetails", { appId: "bd.myai.app" }],
    ["setIcon", image],
    ["waitForRefresh"],
...
test("uses a stable per-brand AppUserModelID to avoid the installed shortcut icon", () => {
  ...
  assert.match(first, /^bd\.myai\.app\.brand\.[a-f0-9]{16}$/);
...
test("does not refresh the taskbar button before the boot window is shown", async () => {
  ...
  await applyWindowsTaskbarIcon({
    ...
    appId: "bd.myai.app.brand.1234",
    appIconPath: "C:\\brand.ico",
    relaunchCommand: "C:\\myai.exe",
    relaunchDisplayName: "myai",
...
test("builds a per-user Start Menu shortcut with the branded Windows identity", () => {
  ...
  assert.equal(windowsInstalledShortcutFileName("myai"), "myai.lnk");
  assert.deepEqual(windowsBrandShortcutDetails({
    target: "C:\\Program Files\\myai\\myai.exe",
    appId: "bd.myai.app.brand.1234",
    appIconPath: "C:\\Users\\Admin\\brand-icon.ico",
    appName: "myai",
  }), {
    target: "C:\\Program Files\\myai\\myai.exe",
    cwd: "C:\\Program Files\\myai",
    description: "myai organization desktop",
    icon: "C:\\Users\\Admin\\brand-icon.ico",
    iconIndex: 0,
    appUserModelId: "bd.myai.app.brand.1234",
  });
});
...
test("anchors a packaged shortcut target to the active Windows user profile", () => {
  assert.equal(windowsInstalledExecutablePath({
    packaged: true,
    execPath: "C:\\Windows\\System32\\config\\systemprofile\\AppData\\Local\\Programs\\@openworkdesktop\\myai.exe",
  ...
  }), "C:\\Users\\Administrator\\AppData\\Local\\Programs\\@openworkdesktop\\myai.exe");
});

test("creates a branded shortcut after callers remove stale Windows metadata", () => {
  const calls = [];
  const details = { appUserModelId: "bd.myai.app.brand.1234" };
  const shellApi = {
  ...
  const shortcutPath = "C:\\Users\\Admin\\myai Organization.lnk";
