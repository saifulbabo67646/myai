...
export const OPENWORK_DESKTOP_ID = "bd.myai.app.desktop";
export const OPENWORK_DESKTOP_NAME = "bd.myai.app";
export const OPENWORK_PROTOCOL_MIME = "x-scheme-handler/myai";

const INTEGRATION_STATE_VERSION = 1;
const OWNERSHIP_MARKER = "X-Myai-Managed";
const MANAGED_VERSION_MARKER = "X-Myai-Version";
...
Comment=Run agents, skills, and MCP with myai
...
X-Myai-Distribution=${cleanDesktopValue(distribution)}
X-Myai-AppImage=${cleanDesktopValue(appImagePath)}
`;
}
...
export function createLinuxDesktopIntegration({
  ...
  async function install(options = {}) {
    ...
    if (before.ownership === "external") {
      ...
      return {
        ok: false,
        status,
        error: "The external launcher was not selected for myai:// callbacks.",
      };
    }
    if (
      ...
      return {
        ok: false,
        status: before,
        error: "The canonical myai launcher is externally managed and will not be overwritten.",
      };
    }
    ...
    try {
      ...
      if (!registration.ok) {
        throw new Error(registration.stderr || "xdg-mime could not register myai://.");
      }
      const status = await getStatus();
      if (status.state !== "integrated") {
        throw new Error("The desktop entry was installed, but the desktop did not select it as the myai:// handler.");
      }
    ...
    if (before.ownership !== "openwork") {
      return {
        ...
        error: before.ownership === "external"
          ...
          : "myai does not own a desktop integration to remove.",
      };
    }
  ...
  async function maybePrompt(window) {
    ...
    // The user already accepted this integration, so drift in the files myai
    ...
    const { response, checkboxChecked } = await dialog.showMessageBox(window, {
      type: "question",
      title: "Add myai to your applications?",
      message: "Add myai to your application launcher and register browser sign-in callbacks?",
      detail: `The launcher will use this AppImage in its current location:\n${appImagePath}\n\nIf you move or update it later, myai repairs the launcher automatically. You can change or remove this in Settings → Preferences → AppImage desktop integration.`,
    ...
    if (!result.ok) {
      await dialog.showMessageBox(window, {
        ...
        message: "myai could not complete desktop integration.",
