...
- Connect's public endpoint for external MCP clients is the control-plane origin's \`/mcp/agent\` path; it exposes \`search_capabilities\` and \`execute_capability\`, governed by org membership, roles, policies, and exposure allowlists. The same origin's \`/api/den\` path is an internal same-origin desktop proxy, not an external-client URL. Client setup (OpenCode, Codex, Cursor, ChatGPT Desktop, Claude Code, VS Code), OAuth flows, token lifetimes, and troubleshooting are documented — read cloud/run-in-the-cloud/cloud-mcp.mdx with openwork_docs_read before answering from memory. Read the host from the workspace's configured control-plane origin; never state a host from memory.
